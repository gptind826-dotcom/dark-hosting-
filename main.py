...
# --- Configuration ---
TOKEN = '8758442526:AAHzBsvOry4UXrMl4go-k10_B42sm3ka824' # Replace with your actual token
OWNER_ID = 6448344664 # Replace with your Owner ID
ADMIN_ID = 6315975420 # Replace with your Admin ID (can be same as Owner)
YOUR_USERNAME = '@mahfuj_offcial_143' # Replace with your Telegram username (without the @)
UPDATE_CHANNEL = 'https://t.me/mahfuj_offcial' # Replace with your update channel link

# Verification Channels/Groups
VERIFICATION_CHANNELS = [
    'https://t.me/mahfuj_offcial',
    'https://t.me/ff_tcp_likes_bot',
    'https://t.me/NURROSULAPIS'
]

# Suspicious file patterns (hacking tools)
SUSPICIOUS_PATTERNS = [
    'exploit', 'hack', 'crack', 'keylogger', 'rat', 'backdoor', 
    'malware', 'virus', 'trojan', 'worm', 'ransomware', 'spyware',
    'ddos', 'flood', 'stresser', 'booter', 'phish', 'payload',
    'reverse shell', 'bind shell', 'privilege escalation', 'bypass uac',
    'mimikatz', 'hashdump', 'pass the hash', 'golden ticket',
    'eternalblue', 'bluekeep', 'zerologon', 'printnightmare',
    'nmap', 'masscan', 'zmap', 'hydra', 'medusa', 'ncrack', 'john', 'hashcat',
    'sqlmap', 'havij', 'sqliv', 'bsqlbf', 'xsser', 'beef',
    'metasploit', 'msfvenom', 'empire', 'cobalt strike', 'armitage',
    'wifi', 'aircrack', 'kismet', 'wash', 'reaver', 'bully',
    'keylogger', 'logger', 'record', 'capture', 'sniffer', 'pcap',
    'bypass', 'evade', 'hide', 'stealth', 'obfuscate', 'encrypt'
]
...
# File upload limits
FREE_USER_LIMIT = 10
SUBSCRIBED_USER_LIMIT = 999 # Changed from 10 to 15
...
# Verification system
verified_users = set()  # Users who have completed verification
pending_verification = {}  # {user_id: verification_message_id}
banned_users = set()  # Banned users
user_ban_reason = {}  # {user_id: reason}

# Suspicious file approval system
pending_approval = {}  # {approval_id: {'user_id': user_id, 'file_name': file_name, 'file_path': file_path, 'file_type': file_type, 'admin_chat_id': admin_chat_id}}
user_pending_requests = {}  # {user_id: approval_id}
...
def init_db():
    ...
    try:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS verified_users
                     (user_id INTEGER PRIMARY KEY, verified_date TEXT)''')
        c.execute('''CREATE TABLE IF NOT EXISTS banned_users
                     (user_id INTEGER PRIMARY KEY, reason TEXT, banned_date TEXT)''')
...
def load_data():
    ...
    try:
        ...
        for user_id, reason in c.fetchall():
            banned_users.add(user_id)
            if reason:
                user_ban_reason[user_id] = reason

        conn.close()
        logger.info(f"Data loaded: {len(active_users)} users, {len(user_subscriptions)} subscriptions, {len(admin_ids)} admins, {len(verified_users)} verified, {len(banned_users)} banned.")
    except Exception as e:
...
def ban_user(user_id, reason="No reason provided", admin_id=None):
    """Ban a user"""
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        try:
            ...
            c.execute('INSERT OR REPLACE INTO banned_users (user_id, reason, banned_date) VALUES (?, ?, ?)',
                      (user_id, reason, banned_date))
            conn.commit()
            banned_users.add(user_id)
            user_ban_reason[user_id] = reason
            ...
            logger.warning(f"User {user_id} banned{admin_info}. Reason: {reason}")
...
def unban_user(user_id):
    ...
    with DB_LOCK:
        ...
        try:
            ...
            if user_id in user_ban_reason:
                del user_ban_reason[user_id]
...
def save_verified_user(user_id):
    ...
    with DB_LOCK:
        ...
        try:
            ...
            c.execute('INSERT OR REPLACE INTO verified_users (user_id, verified_date) VALUES (?, ?)',
                      (user_id, verified_date))
            conn.commit()
...
def is_suspicious_file(file_path):
    """Check if file contains suspicious/hacking content"""
    try:
        ...
        for pattern in SUSPICIOUS_PATTERNS:
            if pattern in file_name and pattern not in suspicious_found:
                suspicious_found.append(f"filename_{pattern}")
                
        return len(suspicious_found) > 0, suspicious_found
    except Exception as e:
        logger.error(f"Error checking suspicious file {file_path}: {e}")
        return False, []
...
def get_channel_member_status(user_id, channel_url):
    ...
    try:
        ...
        logger.error(f"Error checking channel status for {user_id} in {channel_url}: {e}")
        return False

def verify_user(user_id, message_id, chat_id):
    """Start verification process"""
    if check_verification_status(user_id):
        bot.edit_message_text("✅ You are already verified!", chat_id, message_id)
        return True
        
    # Check all channels
    missing_channels = []
    for channel in VERIFICATION_CHANNELS:
        if not get_channel_member_status(user_id, channel):
            missing_channels.append(channel)
    
    if not missing_channels:
        # User is in all channels, verify them
        ...
        markup.add(types.InlineKeyboardButton("✅ I've Joined", callback_data="verify_check"))
        bot.edit_message_text(
        ...
        pending_verification[user_id] = message_id
        return False

def handle_verification_check(call):
    """Handle verification check button"""
    ...
    if not missing_channels:
        ...
        channel_list = "\n".join([f"• {ch}" for ch in missing_channels])
        bot.answer_callback_query(call.id, "❌ Still missing some channels!")
        bot.edit_message_text(
            f"❌ You still need to join:\n{channel_list}\n\n"
            f"After joining, click the button below to verify.",
            chat_id, message_id,
            reply_markup=types.InlineKeyboardMarkup().add(
                types.InlineKeyboardButton("✅ I've Joined", callback_data="verify_check")
            )
        )
...
def create_main_menu_inline(user_id):
    ...
    base_buttons = [
        ...
        types.InlineKeyboardButton('✅ Verify', callback_data='verify_start'),
    ...
    if user_id in admin_ids:
        admin_buttons = [
            ...
            types.InlineKeyboardButton('⏳ Pending Approvals', callback_data='pending_approvals')
        ]
        markup.add(base_buttons[0])  # Updates
        markup.add(base_buttons[1])  # Verify
        markup.add(base_buttons[2], base_buttons[3])  # Upload, Check Files
        markup.add(base_buttons[4], admin_buttons[0])  # Speed, Subscriptions
        ...
        markup.add(admin_buttons[6], admin_buttons[7])  # Ban User, Unban User
        markup.add(admin_buttons[8], admin_buttons[9])  # Ban List, Pending Approvals
        markup.add(admin_buttons[4])  # Admin Panel
        markup.add(base_buttons[5])  # Contact
    else:
        markup.add(base_buttons[0])
        markup.add(base_buttons[1])
        markup.add(base_buttons[2], base_buttons[3])
        markup.add(base_buttons[4])
        markup.add(types.InlineKeyboardButton('📊 Statistics', callback_data='stats'))
        markup.add(base_buttons[5])
    return markup
...
def create_approval_buttons(approval_id):
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    ...
    try:
        ...
        bot.reply_to(message, f"✅ Files extracted. Checking main script: `{main_script_name}`...", parse_mode='Markdown')

        # Check for suspicious content
        is_suspicious, found_patterns = is_suspicious_file(main_script_path)
        if is_suspicious and user_id not in admin_ids and user_id != OWNER_ID:
            # Require admin approval
            approval_id = f"approval_{int(time.time())}_{user_id}"
            pending_approval[approval_id] = {
                'user_id': user_id,
                'file_name': main_script_name,
            ...
            # Notify all admins
            patterns_text = ", ".join(found_patterns[:5])
            for admin_id in admin_ids:
                try:
                    bot.send_message(
                        admin_id,
                        f"⚠️ Suspicious File Approval Needed\n\n"
                        f"User: {user_id}\n"
                        f"File: {main_script_name}\n"
                        f"Type: {file_type}\n"
                        f"Suspicious patterns: {patterns_text}\n\n"
                        f"Approve or reject?",
                        reply_markup=create_approval_buttons(approval_id)
                    )
                except:
                    pass
            ...
            bot.reply_to(
                message,
                f"⚠️ Your file contains patterns that might be suspicious.\n"
                f"Admins have been notified for approval.\n"
                f"You'll be notified when approved."
            )
        else:
            # Auto-start for admins or clean files
            if file_type == 'py':
...
def handle_js_file(file_path, script_owner_id, user_folder, file_name, message):
    try:
        ...
        # Check for suspicious content
        is_suspicious, found_patterns = is_suspicious_file(file_path)
        if is_suspicious and script_owner_id not in admin_ids and script_owner_id != OWNER_ID:
            # Require admin approval
            approval_id = f"approval_{int(time.time())}_{script_owner_id}"
            pending_approval[approval_id] = {
            ...
            user_pending_requests[script_owner_id] = approval_id
...
# --- Approval Handling ---
def handle_approval(call, approval_id, approve):
    """Handle admin approval/rejection of suspicious files"""
    if call.from_user.id not in admin_ids:
        bot.answer_callback_query(call.id, "⚠️ Admin only.", show_alert=True)
        return
        
    if approval_id not in pending_approval:
        bot.answer_callback_query(call.id, "❌ Approval request expired.", show_alert=True)
        bot.edit_message_text("❌ Approval request expired.", call.message.chat.id, call.message.message_id)
        return
        
    data = pending_approval[approval_id]
    user_id = data['user_id']
    file_name = data['file_name']
    file_path = data['file_path']
    file_type = data['file_type']
    found_patterns = data.get('found_patterns', [])
    ...
    if approve:
        ...
        try:
            bot.send_message(
                user_id,
                f"✅ Your file '{file_name}' has been approved and is now running!"
            )
        except:
            pass
        ...
        bot.edit_message_text(
            f"✅ Approved file from user {user_id}\nFile: {file_name}",
        ...
        # Notify user
        patterns_text = ", ".join(found_patterns[:3])
        try:
            bot.send_message(
                user_id,
                f"❌ Your file '{file_name}' has been rejected.\n"
                f"Reason: Contains suspicious patterns ({patterns_text})"
            )
        except:
            pass
            
        bot.answer_callback_query(call.id, "❌ File rejected and deleted.")
        bot.edit_message_text(
            f"❌ Rejected file from user {user_id}\nFile: {file_name}",
            call.message.chat.id, call.message.message_id
        )
    
    # Clean up
    if user_id in user_pending_requests:
        del user_pending_requests[user_id]
    del pending_approval[approval_id]
# --- End Approval Handling ---
...
def _logic_send_welcome(message):
    ...
    if is_banned(user_id):
        reason = user_ban_reason.get(user_id, "No reason provided")
        bot.send_message(chat_id, f"🚫 You are banned from using this bot.\nReason: {reason}")
        return
    ...
    if user_id not in admin_ids and user_id != OWNER_ID and not check_verification_status(user_id):
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("✅ Verify Now", callback_data="verify_start"))
        bot.send_message(
            chat_id,
            "⚠️ You need to verify before using the bot!\n\n"
    ...
    welcome_msg_text = (f"〽️ Welcome, {user_name}!\n\n🆔 Your User ID: `{user_id}`\n"
                        ...
                        f"✅ Verified: {'Yes' if check_verification_status(user_id) else 'No'}\n"
...
def _logic_verify(message):
    ...
    msg = bot.reply_to(message, "🔄 Checking verification status...")
    verify_user(user_id, msg.message_id, chat_id)
...
def _logic_upload_file(message):
    ...
    # Check verification
    if user_id not in admin_ids and user_id != OWNER_ID and not check_verification_status(user_id):
        bot.reply_to(message, "⚠️ You need to verify first! Use /verify or click Verify button.")
        return
...
def _logic_ban_user(message):
    ...
    msg = bot.reply_to(
        ...
        "Format: `/ban user_id reason` or `/ban @username reason`\n"
...
def _logic_ban_list(message):
    ...
    for user_id in sorted(banned_users):
        reason = user_ban_reason.get(user_id, "No reason")
        ban_list += f"• `{user_id}` - {reason}\n"
    
    # Split if too long
    if len(ban_list) > 4000:
        parts = [ban_list[i:i+4000] for i in range(0, len(ban_list), 4000)]
        for part in parts:
            bot.reply_to(message, part, parse_mode='Markdown')
    else:
        bot.reply_to(message, ban_list, parse_mode='Markdown')
...
def _logic_pending_approvals(message):
    ...
    approval_list = "⏳ Pending Approvals:\n\n"
    for approval_id, data in pending_approval.items():
        approval_list += f"• User: `{data['user_id']}`\n"
        approval_list += f"  File: {data['file_name']}\n"
        approval_list += f"  Type: {data['file_type']}\n"
        approval_list += f"  Patterns: {', '.join(data.get('found_patterns', [])[:3])}\n\n"
    
    bot.reply_to(message, approval_list, parse_mode='Markdown')
...
def process_ban_user(message):
    ...
    try:
        ...
        user_identifier = parts[0].replace('/ban', '').strip() if parts[0].startswith('/ban') else parts[0]
        reason = parts[1] if len(parts) > 1 else "No reason provided"
        
        # Extract user ID from username if needed
        if user_identifier.startswith('@'):
            ...
            try:
                ...
                bot.reply_to(message, f"❌ Could not find user {user_identifier}")
                return
        else:
            try:
                user_id = int(user_identifier)
            except ValueError:
                bot.reply_to(message, "❌ Invalid user ID format. Use numeric ID or @username")
                return
        ...
        if ban_user(user_id, reason, admin_id):
            bot.reply_to(message, f"✅ User `{user_id}` banned.\nReason: {reason}", parse_mode='Markdown')
        else:
            bot.reply_to(message, f"❌ Failed to ban user `{user_id}`", parse_mode='Markdown')
            
    except Exception as e:
        logger.error(f"Error in process_ban_user: {e}")
        bot.reply_to(message, "❌ Error processing ban. Check format.")
...
def process_unban_user(message):
    ...
    try:
        ...
        # Parse user ID
        user_identifier = parts[0].replace('/unban', '').strip() if parts[0].startswith('/unban') else parts[0]
        ...
        if unban_user(user_id):
            ...
            bot.reply_to(message, f"❌ Failed to unban user `{user_id}`", parse_mode='Markdown')
            
    except Exception as e:
        logger.error(f"Error in process_unban_user: {e}")
        bot.reply_to(message, "❌ Error processing unban. Check format.")
...
def command_send_welcome(message): 
    ...
    if is_banned(message.from_user.id):
        reason = user_ban_reason.get(message.from_user.id, "No reason provided")
...
def command_ban(message): 
    _logic_ban_user(message)

@bot.message_handler(commands=['unban'])
def command_unban(message): 
    _logic_unban_user(message)

@bot.message_handler(commands=['banlist'])
def command_ban_list(message): 
    _logic_ban_list(message)
...
BUTTON_TEXT_TO_LOGIC = {
    ...
    "🚫 Ban User": _logic_ban_user,
    "✅ Unban User": _logic_unban_user,
    "📋 Ban List": _logic_ban_list,
    "⏳ Pending Approvals": _logic_pending_approvals,
}

@bot.message_handler(func=lambda message: message.text in BUTTON_TEXT_TO_LOGIC)
def handle_button_text(message):
    # Check if user is banned first (except for verify command)
    if is_banned(message.from_user.id) and message.text != "✅ Verify":
        bot.reply_to(message, "🚫 You are banned from using this bot.")
        return
...
def handle_callbacks(call):
    ...
    # Check if user is banned (allow some basic callbacks)
    if is_banned(user_id) and data not in ['back_to_main', 'speed', 'stats', 'verify_start', 'verify_check']:
        bot.answer_callback_query(call.id, "🚫 You are banned.", show_alert=True)
        return

    if bot_locked and user_id not in admin_ids and data not in ['back_to_main', 'speed', 'stats', 'verify_start', 'verify_check']:
    ...
    try:
        ...
        elif data == 'verify_start': handle_verification_start(call)
        elif data == 'verify_check': handle_verification_check(call)
        # Approval callbacks
        elif data.startswith('approve_'): 
            approval_id = data.replace('approve_', '')
            handle_approval(call, approval_id, True)
        elif data.startswith('reject_'): 
            approval_id = data.replace('reject_', '')
            handle_approval(call, approval_id, False)
        # --- Admin Callbacks ---
        ...
        elif data == 'ban_list': admin_required_callback(call, ban_list_callback)
        elif data == 'pending_approvals': admin_required_callback(call, pending_approvals_callback)
        else:
...
def upload_callback(call):
    ...
    # Check verification
    if user_id not in admin_ids and user_id != OWNER_ID and not check_verification_status(user_id):
        bot.answer_callback_query(call.id, "⚠️ You need to verify first!", show_alert=True)
        return
...
def handle_verification_start(call):
    user_id = call.from_user.id
    chat_id = call.message.chat.id
    message_id = call.message.message_id
    ...
    verify_user(user_id, message_id, chat_id)
...
def ban_list_callback(call):
    bot.answer_callback_query(call.id)
    _logic_ban_list(call.message)
...
if __name__ == '__main__':
    logger.info("="*40 + "\n🤖 Bot Starting Up...\n" + f"🐍 Python: {sys.version.split()[0]}\n" +
                ...
                f"✅ Verification Channels: {len(VERIFICATION_CHANNELS)}\n" + "="*40)
    keep_alive()
